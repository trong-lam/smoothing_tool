...
def recieve_img(filename, activate_normalize=None):
    if request.method == 'POST':
        ...
        if file:
            ...
            #print(" normalized_pred_img ", type(normalized_pred_img))
                # _, im_arrv = cv2.imencode('.png', normalized_pred_img)  # im_arr: image in Numpy one-dim array format.
            # im_bytes = im_arrv.tobytes()
            ...
            #     mimetype='image/png'
            # )
            # with open('linhtinh.txt','w') as f:
            #     f.write(str(im_b64))
            # print("sadsa")
            # return make_response(jsonify(
            #     {"message": 'success', "data": str(im_b64)[2:-1]}
            # ),200)
